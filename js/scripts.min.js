...
d(this).closest(".accordion").find(".accordion-body").not(a).not(".accordion-body.stayopen").slideUp();d(this).toggleClass("active");d(a).slideToggle()}),d(".accordion-toggle").keydown(function(a){32==a.keyCode&&(a=d(this).attr("href"),d(this).closest(".accordion").find(".accordion-toggle").not(d(this)).removeClass("active"),d(this).closest(".accordion").find(".accordion-body").not(a).not(".accordion-body.stayopen").slideUp(),d(this).toggleClass("active"),d(a).slideToggle())}),function(){if(window.location.hash&&
"#nav"!==window.location.hash){var a=window.location.hash.split("_")[0],b=window.location.hash.split("_")[1];"#collapse"==a?d.isNumeric(b)&&(a=h.find("#collapse_"+b),a.closest(".accordion").parent().closest(".accordion-group")&&($upper=a.closest(".accordion").parent().closest(".accordion-group"),$upper.find(".accordion-toggle").addClass("active"),$upper.find(".accordion-body").show(),$upper.find(".accordion-toggle").children().find(".accordion-toggle").removeClass("active"),$upper.find(".accordion-body").children().find(".accordion-body").hide()),
a.find(".accordion-toggle").addClass("active"),a.show(),a=a.offset(),a=a.top-220,d("html,body").animate({scrollTop:a},"slow")):(a=window.location.hash.split("#")[1],a=encodeURIComponent(a),a=d('a[name="'+a+'"]'),a.closest(".accordion").parent().closest(".accordion-group")&&($upper=a.closest(".accordion-group"),$upper.find(".accordion-toggle").addClass("active"),$upper.find(".accordion-body").show(),$upper.find(".accordion-toggle").children().find(".accordion-toggle").removeClass("active"),$upper.find(".accordion-body").children().find(".accordion-body").hide(),
a.addClass("active"),a.show(),a=a.offset(),a=a.top-220,d("html,body").animate({scrollTop:a},"slow")))}}());d("#content table").wrap('<div class="table-wrapper" />').wrap('<div class="scrollable" />');f();d(window).scroll(f);d(".organigram .has-sub").each(function(){d(this).prepend('<span class="toggle-icon"></span>');d(this).children("ul").hide()});d(".organigram .has-sub .toggle-icon").bind("click",function(a){a.preventDefault();d(this).closest(".has-sub").toggleClass("active");d(this).closest(".has-sub").children("ul").slideToggle()});
d("#nav > .nav > li > a").on("touchstart ontouchstart",function(a){if(d(this).parent().hasClass("has-sub")){if(d(this).hasClass("clicked-once"))return!0;d("#nav > .nav > li > a").removeClass("clicked-once");d("#nav > .nav > li").removeClass("focus");d(this).addClass("clicked-once");d(this).parent("li").addClass("focus");a.preventDefault();return!1}return!0});d("#mainnav-toggle").each(function(){var a=d('<button id="mainnav-toggle" type="button" aria-controls="nav" aria-haspopup="true" aria-expanded="false"/>').append(this.innerHTML).click(function(){this._isExpanded=
!this._isExpanded;d(this).attr("aria-expanded",this._isExpanded?"true":"false");d("#logo").toggle(!this._isExpanded)});d(this).replaceWith(a);$backdrop=d('<div id="menu-backdrop" aria-hidden="true"/>').click(function(){a.trigger("click")});d("#nav").after($backdrop)});x=document.getElementById("search-toggle");x._expanded=!1;x._toggleSearch=function(a){this._expanded=a;h.toggleClass("search-toggled",this._expanded);this.setAttribute("aria-expanded",this._expanded?"true":"false");d("#headsearchinput")[this._expanded?
"focus":"blur"]()};d(x).bind("click",function(a){a.preventDefault();this._toggleSearch(!this._expanded)});var a=d(".nav > .has-sub > a + .nav-flyout"),g=function(){var a=this||null,b=!1;d(".nav > .has-sub > [type=button]").each(function(n,c){c._isExpanded=a===c?!c._isExpanded:!1;d(c).attr("aria-expanded",c._isExpanded?"true":"false");b=b||c._isExpanded;c.nextElementSibling.scrollTop=0});r.toggleClass("flyout-scrolling",b)};a.each(function(a,b){a="_"+Math.random().toString(36).substr(2,9);b.$_link=
d(b.previousSibling);b.$_button=d('<button type="button" aria-controls="'+a+'" aria-haspopup="true" aria-expanded="false" aria-hidden="true"/>').text(b.$_link.text()).click(g);d(b).before(b.$_button).attr("id",a)});x=d("#hero .hero-navigation").clone().attr("aria-label","Quicklink Navigation");d('<div class="cloned-hero-nav"></div>').append(x).prependTo("#footer");var l=d(".sidebar-subnav");if(l=l.length?l[0]:null)l._origParentNode=l.parentNode;var e=d("ul.meta-nav"),m=e.length?e.parent():null,n=
null;x=function(){var a=768>window.innerWidth;a!==n&&(n=a,w&&z&&(a?h.hasClass("visiblelogo")||(h.addClass("visiblelogo"),a=h.hasClass("nologo")?"":d(".branding p.sitetitle img").attr("alt"),d(".branding p.sitetitle img").after('<span class="visiblelogo">'+a+"</span>")):h.hasClass("visiblelogo")&&(h.removeClass("visiblelogo"),d(".visiblelogo").remove())),h[n?"removeClass":"addClass"]("responsive-large")[n?"addClass":"removeClass"]("ismobile"),c(v||n),g(),l&&(n?document.getElementById("nav").appendChild(l):
l._origParentNode.insertBefore(l,l._origParentNode.firstChild)),e.length&&(n?d("#nav").append(e.attr({role:"navigation","aria-label":"Portal links"})):(d(".meta-links").append(e.attr({role:null,"aria-label":null})),m.append(e))))};d(window).on("resize",x);x();d(".sorttable").tablesorter()});
